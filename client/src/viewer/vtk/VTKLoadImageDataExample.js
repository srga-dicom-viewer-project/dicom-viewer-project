...
function getImageIds() {
  const ROOT_URL =
    'https://dicom-viewer-project-bucket.s3.us-west-1.amazonaws.com/';//'https://s3.amazonaws.com/IsomicsPublic/SampleData/QIN-H%2BN-0139/PET-sorted/';

  const imageNames = [];

  for (let i = 0; i < 9; i++) {
    imageNames.push('case4d_00' + i + '.dcm');
  }

  // Shuffle the image names to test that src/lib/data/sortDatasetsByImagePosition.js works.
  shuffleImageNames(imageNames);

  return imageNames.map(name => `wadouri:${ROOT_URL}${name}`);
}
